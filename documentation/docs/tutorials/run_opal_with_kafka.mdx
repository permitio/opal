...
Give KafKa and OPAL a few seconds to start up and then run the event update ([see triggering updates](/getting-started/get_started_with_opal_docker_compose_tutorial#step-4-publish-a-data-update-via-the-opal-server)) to check for connectivity.
