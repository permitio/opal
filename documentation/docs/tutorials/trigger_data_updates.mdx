...
In this example a user with the id `some-user-id` was added to some arbitrary user role (`some-new-role`) and we need to propagate this information to our OPAL clients.

- With `$token` being a JWT we generated in [step 1](#datasource-token).
- We publish a data-event regarding the topic `users` pointing clients to `http://mybillingserver.com/users/some-user-id` to obtain the data they need. We also provide the clients with the credentials they'll need to connect to the server (as HTTP authorization headers).

```
curl --request POST 'https://opal.yourdomain.com/data/config' \
--header 'Authorization: Bearer $token' \
--header 'Content-Type: application/json' \
--data-raw '{
  "reason": "someone-updated-this-user",
  "entries": [{
    "url": "http://mybillingserver.com/users/some-user-id"
    "topics": [ "users" ],
    "data": {
      "role": "some-new-role"
    },
    ...
    "save_method": "PUT",
    "dst_path": "/users/some-user-id"
  }]
}'
```

- By providing the data for this entry directly in the `data` field the clients won't have to query `mybillingserver` but instead use the provided data, skipping the extra roundtrip. Nonetheless the `url` field is required.
- `save_method` can be either "PUT" or "PATCH" for update operations. "DELETE" is also supported (see below).
- `dst_path` configures where the data will be saved in OPA.
- `reason` is optional but recommended.

Delete example:

The same endpoint can be used to delete data from the policy store:
...
--header 'Content-Type: application/json' \
--data-raw '{
  "reason": "someone-deleted-this-user",
  "entries": [{
    "topics": [ "users" ],
    "save_method": "DELETE",
    "dst_path": "/users/some-user-id"
