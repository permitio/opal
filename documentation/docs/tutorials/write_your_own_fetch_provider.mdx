...
The one change that breaks at **import time**, before any of your code runs, is a
bare `@root_validator` - it raises `PydanticUserError`. Replace it with
`@model_validator(mode="before")` or `@model_validator(mode="after")`.

The rest are quieter:

- A bare `Optional[X]` annotation is **required** under v2. Write `Optional[X] =
  None` when you want the field to be optional - otherwise a config that used to
  default to `None` becomes a mandatory field.
- `@validator` still works through a deprecation shim; prefer `@field_validator`.
- An inner `class Config` still works through a shim; prefer `model_config =
  ConfigDict(...)`.
- `.dict()` / `.parse_obj()` still work through shims; prefer `.model_dump()` /
  `.model_validate()`. They are removed in pydantic v3.
