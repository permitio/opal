...
:::note pydantic v2
Since OPAL 0.9, `opal-common` requires **pydantic v2**. Custom fetch providers are
pydantic models, so they must be v2-compatible: use `@field_validator` /
`@model_validator` instead of `@validator` / `@root_validator`, `model_config =
ConfigDict(...)` instead of an inner `class Config`, and `.model_dump()` /
`.model_validate()` instead of `.dict()` / `.parse_obj()`. Note also that a bare
`Optional[X]` annotation is **required** under v2 - write `Optional[X] = None` when
you want the field to be optional.
:::
