...
:::warning Breaking change in OPAL 0.10: string fields are no longer coerced
OPAL 0.10 moves to pydantic v2, which **no longer converts numbers into strings**.
Bodies that OPAL 0.9 accepted are now rejected with a `422`.

This affects any backend posting to `POST /data/config` (and
`POST /scopes/{scope_id}/data/update`). The string-typed fields are `reason`,
and per entry `url`, `dst_path`, `save_method` and each item of `topics`:

```jsonc
// rejected from 0.10 - 422 "Input should be a valid string"
{"reason": 12345, "entries": [{"url": "https://api/data", "dst_path": 7}]}

// correct
{"reason": "12345", "entries": [{"url": "https://api/data", "dst_path": "7"}]}
```

A numeric `reason` - a ticket or revision number - is the most likely to bite.
Cast these values to strings before sending.
:::
