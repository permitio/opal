...
:::note Breaking change in OPAL 0.10 if you also PUBLISH updates
This page covers the flow where OPAL **fetches** config from your server, which
is unaffected. If your backend also **posts** data updates to OPAL, note that
from 0.10 pydantic v2 no longer coerces numbers to strings and any string-typed
field rejects a numeric value — see
[Triggering Data Updates](/tutorials/trigger_data_updates).
:::
