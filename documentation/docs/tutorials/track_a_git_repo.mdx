...
In general `data.json` files are mostly meant for backward as a backward compatibility to vanilla OPA bundles, and we recommend you use dynamic updates with data-sources-config instead.
If you do use `data.json` here are a few things to note:
