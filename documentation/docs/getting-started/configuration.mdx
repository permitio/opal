...
Soft timeout (seconds) for a single scope git clone/fetch: the awaiting operation is abandoned on timeout (logged, skipped, retried next cycle) while the underlying git call keeps running on its thread until the OS network timeout, so one unreachable repo never blocks boot or other scopes. `0` = no timeout.
