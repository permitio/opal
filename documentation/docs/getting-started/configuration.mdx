...
For more information, see [Cedar and OPAL](/tutorials/cedar) or [OpenFGA and OPAL](/tutorials/openfga).
...
#### OPAL_INLINE_OPENFGA_ENABLED

Default: `False`

Whether OPAL should run OpenFGA by itself in the same container.

For more information, see [OpenFGA and OPAL](/tutorials/openfga).

#### OPAL_INLINE_OPENFGA_EXEC_PATH

Default: `/usr/local/bin/openfga`

The path to the OpenFGA executable.

For more information, see [OpenFGA and OPAL](/tutorials/openfga).

#### OPAL_INLINE_OPENFGA_CONFIG

Default: `{}`

If inline OpenFGA is enabled, provide options for running the OpenFGA server.

For more information, see [OpenFGA and OPAL](/tutorials/openfga).

#### OPAL_INLINE_OPENFGA_LOG_FORMAT

Default: `none`

The log format for inline OpenFGA. Options: `none`, `minimal`, `http`, `full`.

#### OPAL_OPENFGA_STORE_ID

Default: `None`

The OpenFGA store ID that OPAL client will write policy/data updates into.
