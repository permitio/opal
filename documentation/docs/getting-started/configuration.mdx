...
Seconds between two consecutive broadcaster keepalive messages (published by each pod's leader worker on the backbone). Besides keeping idle backbone sessions warm, these are the heartbeat the reader silence watchdog (`OPAL_BROADCAST_READER_SILENCE_TIMEOUT`) listens for: a healthy fleet is never silent for longer than this. `0` disables the keepalive — and with it the silence watchdog, which would otherwise mistake a quiet channel for a dead one. The default was `3600` before 0.9.9; it was lowered so that silence on the backbone means something.

#### OPAL_BROADCAST_READER_SILENCE_TIMEOUT

Default: `180.0`

Reader liveness by silence. If the broadcaster reader receives no backbone message of any kind (keepalives included) for this many seconds, the listening connection is treated as dead — it is terminated so the connection pool cannot reuse it — and the normal reconnect + client-resync path runs. This is what catches a *half-open* backbone connection (for example after a database failover that moves the endpoint to another host without closing the old sockets), where nothing else ever fires. A trip does **not** flip `/healthcheck` (the worker is mid-reconnect, exactly like a peer-announced disconnect, and its clients can still fetch bundles and data); it is visible as the `opal_server.broadcaster_reader_silent` gauge (per pid, 1 while tripped) and the `opal_server.broadcaster_silence_trips` counter. The clock is unarmed until this worker has heard the backbone at least once (so a slow boot, or a single pod whose leader has not published yet, is never counted as silence); from then on every subscription is armed as soon as it is established, with a first-message grace of at least two keepalive intervals, so a reconnect that lands on a stale endpoint and never hears anything is caught too. The time spent fanning an event out to clients is not counted as silence. Must be comfortably above `OPAL_BROADCAST_KEEPALIVE_INTERVAL` (it is raised to twice that if set lower); `0` disables. A trip costs one reconnect and one client resync — the same as a peer-announced disconnect.

#### OPAL_BROADCAST_TCP_KEEPALIVE_ENABLED

Default: `True`

Enable TCP keepalive on the broadcaster's Postgres connections (listen and publish), so a half-open peer is declared dead by the kernel after `OPAL_BROADCAST_TCP_KEEPALIVE_IDLE` + `OPAL_BROADCAST_TCP_KEEPALIVE_INTERVAL` × `OPAL_BROADCAST_TCP_KEEPALIVE_COUNT` seconds (60 s at the defaults) instead of never. Applied through the pool's connection-init hook; fail-open (logged) where the platform refuses the options; Postgres backbones only. Other backends log one INFO line; for them the silence watchdog is the only half-open detector, and it covers a session once the worker has heard the backbone at least once — before that first message nothing detects a half-open listener on those backbones (on Postgres, TCP keepalive does).

#### OPAL_BROADCAST_TCP_KEEPALIVE_IDLE

Default: `30`

Seconds of idleness before the first TCP keepalive probe.

#### OPAL_BROADCAST_TCP_KEEPALIVE_INTERVAL

Default: `10`

Seconds between TCP keepalive probes once the first went unanswered.

#### OPAL_BROADCAST_TCP_KEEPALIVE_COUNT
