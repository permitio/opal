...
## How to Configure OPAL

#### Environment Variables (Recommended)
Set environment variables prefixed with `OPAL_`:
...
2. **Environment variables** (prefixed with `OPAL_`)
3. **Configuration files** (`.env` or `.ini`)
4. **Default values** (lowest priority)

#### CLI Arguments
Convert CLI arguments to environment variables:
- `--server-url` → `OPAL_SERVER_URL`
- `--data-config-sources` → `OPAL_DATA_CONFIG_SOURCES`

#### Production Security Checklist

:::danger
- **Always use HTTPS** for OPAL server
- **Enable JWT verification** with proper keys
- **Set a master token** for authentication
- **Store secrets in a secure vault** (AWS Secrets Manager, HashiCorp Vault, etc.)
- **Never commit secrets** to your repository
:::

Read more about [running the OPAL Server & Client in Secure Mode](/getting-started/running-opal/as-python-package/secure-mode-setup.mdx).
