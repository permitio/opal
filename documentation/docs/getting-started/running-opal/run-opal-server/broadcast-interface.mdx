...
While the backbone is unreachable, cross-server fan-out is paused but client websocket connections are kept alive; updates published during the outage are delivered once the backbone reconnects (OPAL keeps no replay buffer, so a client that itself reconnects re-fetches the full policy/data state).
