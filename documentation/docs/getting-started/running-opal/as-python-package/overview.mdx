...
:::warning IMPORTANT
This guide requires **Python 3.10 to 3.13** for the client, and **Python 3.10 to 3.12** for the server (its pinned `pygit2` ships no Python 3.13 wheel yet). On 3.13, `pip install opal-server` does not fail - it silently installs the old 0.9.6 - so use 3.12 or lower for the server.
