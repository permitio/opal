...
| `test_redis_wiped_boot_reclaims_clones` | **gate (orphan sweep, PR3)** | FAILS without PR3's orphan sweep — after a scope-store wipe, on-disk clones referencing nothing are never reclaimed; same missing-sweep class as the orphan-dir gate. Reclaiming on a zero-scope read is opt-in (`OPAL_SCOPES_ORPHAN_SWEEP_RECLAIM_ON_EMPTY_STORE`, set bed-wide in `docker-compose.yml`) because the sweep cannot tell this deliberate wipe from a misdirected store |
