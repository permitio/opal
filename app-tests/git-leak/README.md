...
| `test_orphan_clone_dir_is_reclaimed` | **gate (orphan sweep, PR3)** | PASSES since PR3's orphan sweep — fails without it, where a clone dir with no live scope is never reclaimed. Runs with a live scope present (the production shape), asserts the sweep leaves that live clone alone, and asserts the dir SURVIVES the first pass: a reclaim requires the dir to look orphaned on consecutive passes, so the test triggers two |
...
| `test_shard_reconfig_still_serves_but_orphans_old_clones` | **gate (S5, orphan sweep, PR3)** | PASSES, both halves — serving survives a `SCOPES_REPO_CLONES_SHARDS` reconfig (re-clone under new ids) and the old-shard dirs are reclaimed by the sweep (two triggers: the first pass corroborates). A reshard orphans many dirs at once, so `SCOPES_ORPHAN_SWEEP_MAX_RECLAIM_PER_PASS` decides how many passes the drain takes — raise it to drain faster |
