...
docker-compose -f docker-compose-with-everything.yml --env-file .env up --force-recreate --remove-orphans -d
sleep 10
...
docker-compose -f docker-compose-with-everything.yml logs opal_client | grep -q "Connected to PubSub server"
docker-compose -f docker-compose-with-everything.yml logs opal_client | grep -q "Got policy bundle"
docker-compose -f docker-compose-with-everything.yml logs opal_client | grep -q 'PUT /v1/data/static -> 204'
if docker-compose -f docker-compose-with-everything.yml logs opal_client | grep -q 'ERROR'; then
...
sleep 2
docker-compose -f docker-compose-with-everything.yml logs opal_client | grep -q 'PUT /v1/data/users/bob/location -> 204'
...
sleep 2
docker-compose -f docker-compose-with-everything.yml logs opal_client | grep -q 'PUT /v1/data/users/alice/location -> 204'
