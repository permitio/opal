...
def test_pull_with_new_commits(
    ...
    """
    remote_repo: Repo = local_repo # local repo, the 'origin' remote of 'local_repo_clone'
    repo: Repo = local_repo_clone # local repo, cloned from 'local_repo'

    tracker = BranchTracker(repo=repo)
    most_recent_commit_before_pull: Commit = repo.head.commit

    assert most_recent_commit_before_pull == tracker.latest_commit == tracker.prev_commit
    ...
    assert remote_repo.head.commit == repo.head.commit == latest == tracker.latest_commit

def test_tracked_branch_does_not_exist(
    local_repo: Repo
):
    """
...
def test_tracked_remote_does_not_exist(
    local_repo_clone: Repo
):
    """
