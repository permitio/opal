...
class NamedLock:
    """
    creates a a file-lock (can be a normal file or a named pipe / fifo),
    and exposes a context manager to try to acquire the lock asyncronously.
    """
    def __init__(self, path: str, attempt_interval: float = DEFAULT_LOCK_ATTEMPT_INTERVAL):
    ...
    async def __aenter__(self):
        """
        using the lock as a context manager will try to acquire
        the lock until successful (without timeout)
        """
        await self.acquire()
        return self
    ...
    async def acquire(self, timeout: Optional[int] = None):
        ...
        if unsuccessful, will sleep and then try again after the attempt interval.
    ...
    def _acquire(self) -> bool:
        """
        tries to acquire the lock, returns immediately regardless of success.
