...
async def test_was_fetched_after_is_per_source(tmp_path):
    ...
    GitPolicyFetcher.repos_last_fetched[fetcher_x._source_id] = now + datetime.timedelta(
        seconds=1
    )
...
async def test_force_fetch_not_downgraded_by_sibling_source(monkeypatch, tmp_path):
    ...
    GitPolicyFetcher.repos_last_fetched[fetcher_x._source_id] = (
    ...
    assert await fetcher_y._should_fetch(
