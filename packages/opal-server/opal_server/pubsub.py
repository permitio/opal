...
class PubSub(Publisher):
    ...
    async def start(self):
        if self.broadcaster is not None:
            ...
            self._wait_for_broadcaster_closed = asyncio.create_task(
                self.wait_until_done(self._disconnect_callback)
            )
    ...
    async def wait_until_done(self, done_callback: Optional[Coroutine] = None):
        if self.broadcaster is not None:
            await self.broadcaster.wait_until_done()
        if done_callback is not None:
            # TODO: Improve because main loop also wait for broadcaster to be done
            await done_callback
    ...
    async def publish(self, topics: TopicList, data: Any = None):
        await self._publish_pool.add_task(self.publish_sync(topics, data))
