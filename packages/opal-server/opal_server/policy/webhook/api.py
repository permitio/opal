    ...
    async def trigger_webhook(
        ...
        if source_type == PolicySourceTypes.Git:
            ...
            if opal_server_config.POLICY_REPO_URL is not None and opal_server_config.POLICY_REPO_URL in urls:
                logger.info("triggered webhook on repo: {repo}", repo=opal_server_config.POLICY_REPO_URL, hook_event=event)
                if event == 'push':
                ...
                return { "status": "ok", "event": event, "repo_url": opal_server_config.POLICY_REPO_URL }

        elif source_type == PolicySourceTypes.Api:
            logger.info("Triggered webhook to check API bundle URL")
            await pubsub_endpoint.publish(opal_server_config.POLICY_REPO_WEBHOOK_TOPIC)
            return {"status": "ok", "event": "webhook_trigger", "repo_url": opal_server_config.POLICY_BUNDLE_URL}
