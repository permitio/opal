...
class OpalServerConfig(Confi):
    ...
    SCOPES_GIT_BACKOFF_MAX_SECONDS = confi.float(
        ...
        "does every duplicate scope sharing that repo — on one production "
        "cluster 19 fast-failing sources produced 4,579 clone attempts in "
        "three hours, 2,410 of them from a single repo shared by ~56 scopes. "
        ...
        "in-memory and per process — it resets on restart, and a forked worker "
        "inherits whatever the pre-fork preload recorded, which is deliberate: "
        "the leader must not re-hammer repos that already failed at boot. "
