...
class OpalServerConfig(BaseSettings):
    class Config:
        env_prefix = "OPAL_"
    ...
    def auth_private_key(cls, v, values):
        return cast_private_key(
            v, values["AUTH_PRIVATE_KEY_FORMAT"], values["AUTH_PRIVATE_KEY_PASSPHRASE"]
        )
    ...
    POLICY_REPO_WEBHOOK_PARAMS: GitWebhookRequestParams = GitWebhookRequestParams(
