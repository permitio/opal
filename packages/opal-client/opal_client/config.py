...
from opal_common.config import CommonStatisticsConfig, OpalSettings, opal_common_config
...
class OpalClientConfig(OpalSettings):
    class PolicyStoreConfig(BaseSettings):
        ...
        # TODO: Further break down OPA/Cedar configuratins into their own sections
    ...
    policy_store: PolicyStoreConfig = PolicyStoreConfig()
    ...
    class ClientStatisticsConfig(CommonStatisticsConfig):
        OPAL_CLIENT_STAT_ID: str = Field(
            None,
            description="Unique client statistics identifier",
        )

    statistics: ClientStatisticsConfig = ClientStatisticsConfig()
    ...
    def on_load(self):
        # LOGGER
        if self.policy_store.INLINE_OPA_LOG_FORMAT == EngineLogFormat.NONE:
            opal_common_config.logging.LOG_MODULE_EXCLUDE_LIST.append(
                "opal_client.opa.logger"
            )
            # re-assign to apply to internal confi-entries as well
            opal_common_config.logging.LOG_MODULE_EXCLUDE_LIST = (
                opal_common_config.logging.LOG_MODULE_EXCLUDE_LIST
            )
